...
To meet urgent deadlines, systems must prioritize and ensure the timely execution of critical tasks.
...
Unlike traditional thread or process switches, which can occur at any time and require a larger context, coroutine switches happen only at programmer-defined suspension points. 
...
Coroutines on the CPU are implemented using the native x86 calling conventions and the program stack to save and restore continuations dynamically.
Under x86 convention, when a function is invoked, the CPU pushes the return address, the location of the next instruction after the call, onto the stack before jumping to the target function.
...
The callee may freely modify volatile registers, while callee saved registers must be preserved and restored before the function returns.

When execution reaches a return instruction, the CPU pops the return address off the stack and continues execution from that point.
To implement coroutine behaviour, however, the function must be able to yield and later resume.  
This requires saving the callee-saved registers, since their preservation is guaranteed, as well as any additional state such as local variables or registers, that is necessary for continued execution.
These elements can be stored on the stack, allowing a coroutine to pause and continue seamlessly later. 

The Apollo project demonstrates this mechanism by implementing CPU coroutines directly on top of the x86 calling convention, as illustrated in the following code block. 
...
Each stage loads or saves its registers in a specific, defined order. 
...
Unlike CPU coroutines, which can implement context switching by saving and restoring stack frames, GPUs cannot use the exact mechanism, as GPU threads do not maintain conventional stack frames.
Instead, their execution state is managed differently, requiring specialized approaches to preserve and resume coroutine execution.
...
\acsp{GPU}, in contrast, avoid traditional stack frames by aggressively inlining function calls, reducing overhead since function code is readily available and no stack frame is needed. 

Implementing and manipulating deep call stacks on GPUs is primarily handled by the compiler and hardware, leaving very little control to the programmer, which makes such approaches impractical.
For example, CUDA initially did not support recursive functions, which were only introduced later via the \lstinline[language=cuda]`-rdc=true` flag. 
...
Given that call stacks are not effectively or efficiently supported in CUDA, the GPU must support a user-level runtime scheduling mechanism to manage contexts dynamically. 
Due to the nature of kernels executing until completion, the \acs{GPU} coroutine scheduler must be based on a persistent kernel, which can schedule coroutines throughout the application's lifetime.
The coroutines running on the persistent threads will then define suspension points to manually relinquish control to the scheduler, allowing it to switch tasks. 
...
Persistent GPU threads provide the programmer with enhanced control over hardware scheduling by running on top of the hardware scheduler, enabling manual execution decisions that would otherwise be unavailable.  
In addition to this increased flexibility, persistent kernels reduce runtime scheduling overhead.
As hardware resources are preallocated, thread and block configurations are loaded ahead of execution. The runtime avoids repeatedly configuring these settings, allowing kernels to run more efficiently. 
