...
Similar to the CPU coroutines, these GPU coroutines were intended to deliver predictable execution latencies, with the added latency improvements that GPU persistent thread offer. 
...
By restricting the scope from full GPU coroutines to persistent threads within Apollo, this thesis sought an open source implementation that could provide fine grained scheduling control specific to Apollo. 
Most persistent thread implementations, however, are highly application specific and not readily available, which led to selecting an implementation that required more work to make it feasible. 
The only suitable open source implementation identified was LightKer, a research project designed to measure the speedup of using persistent threads compared to sequential kernel launches. 
Implementing LightKer required a complete restructuring of the codebase to support a real time system. 
Although full integration into Apollo and support for coroutines remains unfinished, this thesis extends the real time capabilities of LightKer.

The LightKer implementation itself intended to measure the performance difference between sequential kernel executions and a persistent kernel implementation.
...
However, this application does not support variable tasks or runtime memory transfers, which are necessary to pass arguments and results back and forth. 
As such, the implementation only provides real time support for host device synchronization through a mailbox system. 
Nonetheless, the framework provided a fundamental foundation for the GPU scheduling model. 

\subsection{System Design}

At a high level, there are four important components: the task queue, memory buffers, stream design, and gpu block synchronization.
...
The individual contributions explictely discussed are joined together in the following design architecture below. 
The architectural aspects discuss previously are the main aspects of the work of this thesis, with additional tools and synchronization methods such as the mailboxes being used from the LightKernel project.
