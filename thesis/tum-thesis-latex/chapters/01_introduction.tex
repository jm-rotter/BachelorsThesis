...
Autonomous driving presents a formidable challenge in computing, requiring computationaly complex workloads that heavily rely on \acsp{GPU} for efficient execution, yet traditional kernel scheduling techniques fail to deliver the necessary real time execution predictability, latency, and reliability.
Unlike \acs{CPU} tasks, the CUDA API, used to program NVIDIA \acsp{GPU}, does not provide a mechanism to interrupt executing kernels, limiting real time scheduling and resource management techniques \cite{taskparallelism}.
The lack of an interrupt mechanism, can lead to dangerous \acs{GPU} resource contention, where high priority critical tasks are delayed by low priority tasks. 
Instead, for real time systems, \acs{GPU} scheduling needs to include a mechanism to either suspend kernel execution or explicitly control compute and memory resources. 

This thesis will examine and implemented an approach to \ac{GPU} scheduling using coroutines on persistant threads to reduce latency variability and adress contention issues, ultimately delivering the predictability and reliability necessary for real-time autonomous driving applications. 

\subsection{Necessesity of \acsp{GPU} in Autonomous Driving}
Recently, \acsp{GPU} have revolutionized data processing and machine learning training and inference with their ability to handle massive amounts of data and execute simple, but highly-parallelized computations.
...
Perception gathers and interprets vast amounts of data from numerous sensors, such as \ac{LiDAR} sensors, cameras, radar, and ultrasonic sensors, to identify and classify surrounding objects \cite{apollo}.
