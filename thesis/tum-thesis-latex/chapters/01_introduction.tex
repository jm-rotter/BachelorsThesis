...
The autonomous driving platform, Apollo, which serves as the primary motivation for this thesis, illustrates this challenge \cite{Zhu2018BaiduAA} \cite{apollo}.
...
Historically, early autonomous driving systems addressed real-time requirements using a distributed architecture.
In this approach, major functional modules, such as perception, localization, planning, and control, were mapped to separate compute units, which together formed a processing pipeline \cite{6809196}. 
...
This modular architecture ensured responsiveness and real-time guarantees, albeit at the expense of high hardware costs and increased system complexity.

The rise of increasingly powerful GPUs has enabled a shift toward centralized computing, simplifying hardware and reducing complexity while lowering costs \cite{Tabani2021-gi}. 
...
The compute node integrates a CPU for task scheduling and system control, which assigns computationally-intensive workloads to the GPU.
...
Typical real-time solutions based on an \acs{CPU} architecture ensure system safety under contention by preempting non-critical tasks. 
Tasks requiring high responsiveness can then be executed directly after the preemption of resident processes.
Although native support for this preemption already exists on \acsp{CPU} within both the kernel and user space, \acsp{GPU} currently lack such capabilities. 
...
This internal GPU hardware scheduler is optimized for throughput rather than deterministic execution, which is required by real-time systems. 
...
The inability to natively preempt GPU kernels or enforce strict task priorities renders standard GPU kernel scheduling methods unsuitable for safety-critical, real-time workloads.  
Particularly in the context of autonomous driving, the repercussions of latencies pose a safety hazard. 
...
Persistent kernels are specialized functions that are launched at the start of the application and remain active throughout the system's lifetime. 
