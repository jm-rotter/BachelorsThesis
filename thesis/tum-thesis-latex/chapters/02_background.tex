...
\subsection{Memory and Bandwidth Consideration}

When transfering data from the host, the CUDA API does not have access to the \acs{CPU}'s disk memory and thus requires the memory to be pinned to the \acs{CPU}'s \acs{RAM}.
The CUDA API will manually do this, but host arrays allocated directly in pinned memory using \lstinline[language=cuda]'cudaMallocHost()' or \lstinline[language=cuda]`cudaHostAlloc()` skip the added step of pinning memory and are thus faster. 
However, if the pinned memory is too large, it restricts the rest of the \acs{CPU}'s programm in avaiable \acs{RAM}, which may degrade performance by forcing the swapping of memory to disk storage. 
The device memory is partitioned into main memory, the \acs{VRAM}, with 16 GB as well as the L0, L1 and L2 caches. 
The programmer can decide between three different models of memory management \lstinline[language=cuda]`__constant__, __device__`, and \lstinline[language=cuda]`__shared__` memory.
Constant memory can only be written to from the \acs{CPU}
