...
This background chapter begins by examining the role of \acsp{GPU} in real-time systems, focusing on the architectural and programming constraints that influence scheduling and performance. 
The section then develops into an analysis of real-time programming models for GPUs, considering persistent threads and coroutines as implemented in the autonomous driving system Apollo. 
Futhermore, this section provides the necessary technical background to understand how GPU design influences system behavior and scheduling under real-time constraints.


\section{Real Time Systems}

Real-time systems are designed with strict timing constraints to ensure predictable and safe behavior. \cite{10155700}
These constraints are expressed in terms of the system's ability to meet task deadlines, classified as either soft or hard deadlines.
Hard deadlines are critical to the system's safety, and missing these deadlines can result in potential system failure or unsafe conditions. 
For example, missing the deadline on tasks such as collision avoidance or brake activation severely impacts the system's safety.
Soft deadlines, in contrast, are less critical and can be missed without compromising system integrity. 
To ensure the system's safety, tasks are prioritized based on both their deadline urgency and the criticality of their impact. 
Prioritization in real-time systems often requires preempting soft deadlines or non-urgent tasks to prioritize hard deadlines and critical tasks. 
...
For example, the Stanley autonomous car, which won the 2005 DARPA Grand Challenge, utilized six Pentium processors, among which tasks were divided \cite{Hoffmann_undated-qi}.
...
As deep learning techniques advanced, \acsp{CNN} began to outperform traditional methods in perception tasks such as object detection, lane recognition, and environment understanding.  
This shift necessitated the integration of \acsp{GPU} to handle the increased computational demands. 
Early autonomous driving systems, which relied solely on CPUs, struggled to continue performing well in processing data with CNNs in real-time. 
One of the earliest projects using GPUs for autonomous driving was NVIDIA's 2015 project, which utilized a GPU to train a CNN that could steer a car end-to-end from raw camera input \cite{bojarski2016endendlearningselfdriving}. % https://arxiv.org/pdf/1604.07316
...
\acsp{GPU} deliver a significant increase in throughput over CPUs, by simplifying the thread context to afford greater parallelism.
Initially developed to accelerate graphics rendering, a task that is heavily reliant on parallelizable computations, GPU architectures were designed to support increasing numbers of threads. 
...
To achieve greater performance on these tasks, \acsp{CPU} dedicate a substantial "portion of their transistors to non-computational tasks", specifically control and memory management logic \cite{Owens2007-kp}. 
Features such as prefetching, branch prediction, speculative execution, and out-of-order execution enable CPUs to handle irregular, sequential control flows efficiently.
However, this complexity comes at the cost of reducing the fraction of hardware dedicated purely to computation.
...
Consequently, \acsp{CPU} operate at higher clock rates, while \acsp{GPU} achieve higher throughput by increasing arithmetic intensity.
...
Although core components are named differently, both CPU and GPU threads operate similarly, utilizing an instruction decoder, registers, and an arithmetic unit. 
The differences arise when trying to maximize a single control flow. 
In contrast to \acs{CPU} threads, \acs{GPU} threads execute instructions entirely in order, rely on manual prefetching, and use a simpler, conservative branch predictor, which limits their ability to optimize single-threaded performance. %\cite{stpp}
...
The additional complex logic that \acsp{CPU} use to improve single-threaded applications outperform the single-threaded GPU applications, as seen by the following comparison of single-threaded matrix multiplications in Figure~\ref{fig:singlethreadedgraph} and Figure~\ref{fig:singlethreadedmatrix}.
These figures compare the execution of a matrix multiplication using a single GPU thread versus a single CPU thread on a matrix multiplication task. 
...
As seen in Figures~\ref {fig:singlethreadedgraph} and~\ref {fig:singlethreadedmatrix}, GPUs struggle in applications that fail to utilize their architectural parallelism. 
...
For this thesis, any GPU resident scheduler that employs complex control logic will introduce significant latency overhead. 
Scheduling decisions, task management, and resource assignment are inherently challenging to parallelize, making them inefficient on GPU hardware.
As a result, the scheduling logic is delegated to the CPU, where complex control mechanisms can be executed efficiently. 
Consequently, the GPU's role is reduced to dispatching and executing lightweight tasks, while the CPU handles task queuing and resource allocation.
...
Each \acs{SM} receives work in the form of \acsp{CTA}, which are blocks of threads executing the same instruction code. These define the organization and grouping of threads for execution. 
...
If threads within a warp follow different control flow paths, these threads diverge, and the GPU is forced to execute them sequentially with respect to one another.
...
The Tesla V100 GPU \acs{SM} architecture contains four processing partitions, each with its own complete execution pipeline.
...
In GPU scheduling, oversubscription can improve hardware utilization, albeit at the expense of introducing overhead.
...
Whether this is beneficial depends on the workload goals: oversubscription increases throughput, but it may also add latency to individual tasks.
...
Although the \acs{GPU} oversubscribes the number of warps and threads to hide latencies, the total number of dispatch and scheduling units allows for a maximum of $4 * 32 * 80 = 10840$ instructions issued per cycle when the hardware is fully utilized. 
...
Furthermore, suppose multiple thread blocks with high shared memory demands are scheduled to the same \acs{SM}. In that case, they will saturate the L1 data and shared memory cache, forcing frequent evictions and writebacks to the L2 cache or global memory.  
These hardware limits must be carefully considered when mapping tasks to \acsp{SM}.

Native GPU code handles these constraints automatically, without requiring any input from the programmer.
...
While the exact documentation is not public, this module maps \acsp{CTA} to the individual \acsp{SM} based on a multitude of factors: hardware resources, parallelism, priorities, and dependencies.
Similarly, the global memory and L2 cache utilization are determined by the hardware and are transparent to the programmer.
...
Ideally, the \acsp{SM} execute independently of one another and accumulate answers in global memory, thereby skipping the high memory latency associated with coordinating synchronous work.
...
For tasks to be scheduled on the GPU, the host process must first launch it through the CUDA API, an extension of the C++ programming language that enables the CPU to communicate with and control the GPU. 
...
Since the memory regions are distinct, the CPU must explicitly allocate and transfer data to the GPU using the CUDA API.
Transfers require the CPU memory to be pinned in RAM, as CUDA cannot access the disk directly.
While the CUDA runtime can automatically pin memory, host arrays allocated directly in pinned memory using \lstinline[language=cuda]'cudaMallocHost()' or \lstinline[language=cuda]`cudaHostAlloc()` eliminates this extra step and enables faster transfers.
However, allocating excessive pinned memory reduces the RAM available to other CPU processes, potentially causing paging and degrading system performance. 
Careful management of pinned memory is therefore essential for maximizing data transfer efficiency without compromising the host system's performance. 
...
The code block above illustrates the launch and execution of a simple GPU kernel, as well as the memory allocation scheme used for executing kernel code.
The kernel itself is the execution of the \acs{GPU} device program denoted by the \lstinline[language=cuda]`__global__` function. In contrast, the \lstinline`<<<_, _>>>` syntax enables the programmer to partition their execution tasks across waiting threads specifically. 
...
This is achieved by utilizing the dimensional properties assigned to each thread by the runtime system. 
...
The host array, \lstinline[language=C++]`h_x` is allocated to the CPU stack, which is explicitly copied to the \acs{GPU}.
Trying to pass the array by value, something common in C++ code, seems at first the simplest; however, it poses two separate issues.
Firstly, when passing arrays as parameters, they decay to pointers, which CUDA forbids, as the pointer address passed to the device has no meaning on the GPU.
Secondly, if the array were wrapped in a struct and passed to the function to circumvent the first issue, the array would be allocated independently for every single thread. 
In the example above, the array would be allocated 32 times, each independent of the others, taking up further memory bandwidth and both on-chip and global device memory. 
In this case, each individual thread would get the array passed by value, leading to a total of 32 threads * 1024 floats * 4 bytes per float or 128KiB. 
Instead, the memory is allocated in the device memory, transferred once, and each thread receives only the device pointer \lstinline[language=C++]`d_x`, which can be used to copy the results back to the host.  
...
A stream, represented by the type \lstinline[language=cuda]{cudaStream_t} acts as a handle to a specific command queue, similar to how a file pointer in Linux refers to a particular file descriptor.
Within a stream, operations such as kernel launches, memory copies, and memory set operations are enqueued and executed strictly in the order they are issued, ensuring deterministic behavior.
...
When using multiple streams, the GPU can execute operations concurrently, allowing kernels and memory operations to overlap in time.
By carefully managing streams, developers can expose task parallelism, reduce idle time, and utilize GPU resources more effectively.

The Tesla V100 GPU features two separate hardware copy engines for copying data from the host to the device and vice versa. 
...
Using only one stream for multiple kernels fails to utilize the device's memory bandwidth fully. 
...
Regardless of the ordering of the API calls, the two kernels cannot execute concurrently or share the hardware copy engines, since their execution is serialized by being placed in the same CUDA stream.
